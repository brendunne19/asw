-----------------------------------------------------------------------------
------------- COLLECTING DATASETS FOR MFR SKINCARE MODELS --------------
-----------------------------------------------------------------------------

begin mfr_high_skincare_model_data_prediction_auto(202202, 202301);
END;

--------------------------CREATING TEMP TABLES -----------------------------
DROP TABLE mfr_HIGH_SKINCARE_MODEL_PREDICTION_MEMBERS_auto;
DROP TABLE mfr_HIGH_SKINCARE_MODEL_PREDICTION_DATA_auto;
DROP TABLE mfr_HIGH_SKINCARE_MODEL_PREDICTION_auto;

CREATE TABLE mfr_HIGH_SKINCARE_MODEL_PREDICTION_MEMBERS_auto AS select * from crm_target.b_time where date_key = 20230101;
CREATE TABLE mfr_HIGH_SKINCARE_MODEL_PREDICTION_DATA_auto as select * from crm_target.b_time where date_key = 20230101;
CREATE TABLE mfr_HIGH_SKINCARE_MODEL_PREDICTION_auto AS select * from crm_target.b_time where date_key = 20230101;
----------------------------------- PROC ------------------------------------
create or replace procedure mfr_high_skincare_model_data_prediction_auto (
...
member_list_del := 'DROP TABLE mfr_HIGH_SKINCARE_MODEL_PREDICTION_MEMBERS_auto';
execute immediate (member_list_del);
commit;

member_list := '
CREATE TABLE mfr_HIGH_SKINCARE_MODEL_PREDICTION_MEMBERS_auto AS
(
     SELECT
...
member_data_del := 'DROP TABLE mfr_HIGH_SKINCARE_MODEL_PREDICTION_DATA_auto';
execute immediate(member_data_del);
commit;

member_data :='
CREATE TABLE mfr_HIGH_SKINCARE_MODEL_PREDICTION_DATA_auto AS
SELECT
    CONTACT_KEY
,   member_key
,   TRANSACTION_DT_KEY
...
FROM
    ...
    JOIN CRM_TARGET.B_TIME TM
      ON TM.DATE_KEY = T.TRANSACTION_DT_KEY
    JOIN mfr_HIGH_SKINCARE_MODEL_PREDICTION_MEMBERS_auto M
    ...
    WHERE T.BU_KEY = 16
      ...
      AND MEMBER_SALE_FLAG = ''Y''
      AND KPI_EXCLUSION_FLAG = ''N''
      AND FISCAL_MTH_IDNT BETWEEN '''||ly_start_month||''' AND '''||end_month||'''
    GROUP BY
        M.CONTACT_KEY
    ,   t.member_key
    ,   TRANSACTION_DT_KEY
...
member_level_del := 'DROP TABLE mfr_HIGH_SKINCARE_MODEL_PREDICTION_auto';
execute immediate(member_level_del);
commit;

member_level := '
CREATE TABLE mfr_HIGH_SKINCARE_MODEL_PREDICTION_auto AS
SELECT
...
FROM mfr_HIGH_SKINCARE_MODEL_PREDICTION_DATA_auto T
